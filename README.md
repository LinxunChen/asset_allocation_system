...
这个仓库现在先承载 `卫星仓位 agent`，后面还会继续加入 `核心仓位 agent`。

根目录 `README.md` 只负责做仓库入口和 GitHub 展示；具体文档按 agent 维度拆开，分别在各自目录维护。
...
- 文档维护方式：
  - 根目录保留仓库级 `README.md`
  - 各 agent 在自己的目录下维护子 `README.md`
  - `HANDOFF.md` 仅在需要持续交接的 agent 目录下保留
